...
from awardshow_suite.loader import generate_files
from awardshow_suite import helper
from awardshow_suite import db
from awardshow_suite import awardshow_db
try:
    import easygui
except ImportError:
...
# Award Show Predictions Endpoints
@app.get("/data/awards")
async def get_awards():
    return await awardshow_db.get_awards()


# Room endpoints
@app.get("/data/rooms")
async def get_rooms():
    return await awardshow_db.get_rooms()


@app.get("/data/rooms/{code}")
async def get_room_by_code(code: str):
    room = await awardshow_db.get_room_by_code(code)
    if room:
        return room
...
async def create_room(room: RoomCreate):
    room_id = await awardshow_db.create_room(room.name, room.code)
    if room_id:
        return {"id": room_id}
...
async def delete_room(room_id: int):
    await awardshow_db.delete_room(room_id)
...
async def get_guests(room: Optional[str] = None):
    return await awardshow_db.get_guests(room_code=room)


@app.get("/data/guests_with_scores")
async def get_guests_with_scores(room: Optional[str] = None):
    return await awardshow_db.get_guests_with_scores(room_code=room)


@app.get("/data/guests/{guest_id}")
async def get_guest(guest_id: int):
    guest = await awardshow_db.get_guest_by_id(guest_id)
    if guest:
        return {
            "id": guest.doc_id,
            "name": guest["name"],
...
async def create_guest(guest: GuestCreate):
    guest_id = await awardshow_db.create_guest(
        name=guest.name,
        photo=guest.photo,
...
async def update_guest(guest_id: int, guest: GuestUpdate):
    await awardshow_db.update_guest(
        guest_id=guest_id,
        name=guest.name,
        photo=guest.photo,
        predictions=guest.predictions,
...
async def clear_all_guests():
    await awardshow_db.clear_guests()
    return Response(status_code=204)
